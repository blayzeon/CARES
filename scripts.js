...
CALL RECORDS
STATIC ACCOUNTS
    2085550510 - Missing Payment (CC Auths)
    2085550520 - Refund
    2085550710 - Ticket
    2085550720 - Funds Transfer
    2085550730 - Expired Funds
    2085550810 - Fraud (B. Duty Notes)
    2085550820 - Rates
    2085550510 / 01112085550510 - International
    2085550520 - APOC
    2085550530 - Unlimited Calling
    2085550610 - D5
    2085550620 - Blocking Accounts
    2085550630 - GTL Command
    2085550710 - Call Credit
    2085550720 - Trust
    2085550730 - CNBOT
    2085550530 - SSO
...
function currTime(){
    ...
    return dynamicDate.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true });
}
...
function NewAccount(accountNumber, firstName="", lastName="", address1="", address2="", zipCode="", city="", state="", ivrPasscode="", notes="", authorizedUser="", accountStatus="active", accountType='advancepay', available='0.00', comments="", policy1=false, policy2=false, policy3=false, policy4=false, customerBlock=false, ccBlock=false, created=true) {
...
function addNewComment(commentToAdd){
    ...
    let newComments = `<tr><td>${currDate}${currTime()}</td><td>${caresUser} ${commentToAdd}</td></tr>` + oldComments;
...
function NewTransaction(transactionTotal, adjType, comment="", accountNumber=document.getElementById('lookup-number').value, addedBy=caresUser, transactionDate=currDate, transactionTime=currTime()) {
...
accountList.push(new NewAccount(8889884768, 'GTL', 'IVR', '12021 SUNSET HILLS RD', '#100', '20190', 'RESTON', 'VA', 1234, "GTL Trust IVR #", "none", "active", "advancepay", "0.00", `<tr><td>10/13/2021 9:06:03 AM</td><td>${caresUser} ghost call</td></tr>`, false, false, false, false, false, false));
...
function checkAccount(obj=accountList){
    ...
    obj.push(new NewAccount(accountNumber, ''/* f.name */, '' /* l.name */, '' /* address1 */, '' /* address 2 */, '' /* zipcode */, '' /* city */, '' /* state */, '' /* passcode */ , '' /* notes box */, '' /* authorized user */, 'active' /* account status */, '' /* account type */, '0.00' /* balance */, '' /* comments */, false /* policy1 */,false /* policy2 */,false /* policy3 */,false /* policy4 */, false /* customer block */, false /* cc block */, false /* created? */));
...
function loadAccount(index){
    ...
    // account comments
    changeProperty('account-comments-body', 'innerHTML', accountList[index].comments);
    changeProperty('transactions-body', 'innerHTML', accountList[index].transactionList);
    ...
    if (accountList[index].created == false){
        ...
        changeProperty('load-text', 'innerHTML', `Account not found. Would you like to create one? <span id="button-goes-here"></span>`);
        ...
        createAccBtn.onclick = function(){
            if (document.getElementById('account-type').value == "advancepay"){
                ...
                changeProperty('load-text', 'innerHTML', `Advance Pay account ${accountList[index].accountNumber} loaded successfully.`);
            } else {
...
function loadTransactions(index){
    ...
    let accountTransactions = transactionList.filter(transaction => transaction.accountNumber == accountNumber);
    ...
    for (i = 0; i<accountTransactions.length; i++){
       ...
       if (accountTransactions[i].applied != true){
           ...
           if (accountTransactions[i].ccAuths === true && accountTransactions[i].refunded === false){
           ...
           // refund page
            ...
            if (accountTransactions[i].addedBy != "skip"){
                ...
                <tr>
                    ...
                    <td>${parseFloat(accountTransactions[i].transactionTotal).toFixed(2)}</td>
    ...
    let dumpElms = document.querySelectorAll('#popup-refund-dump .popup-refund-dump');
...
/* [1] */ accountList.push(new NewAccount(2085550510, 'Betty', 'Elderly', '111 COPLIN AVE', '', '85012', 'PHOENIX', 'AZ', '0510', '', 'Charles Elderly', 'lec-inactive', 'advancepay', '0.00', '<tr><td>10/28/2021 4:28:52 PM</td><td>new.trainee Betty Elderly / 0510 / az_doc // account set up & provided payment instructions</td></tr>', true, true, true, true));
accountList[1].email = 'belderly@customer.com';

/* [2] */ accountList.push(new NewAccount(2085550520, 'Jess', 'Baxter', '99 JAMES MARTIN CIR', 'APT 208', '43085', 'WORTHINGTON', 'OH', '', '', '', 'active', 'advancepay', '0.00', '', false, false, false, false));
...
/* [3] */ accountList.push(new NewAccount(2085550610, 'Rae', 'Brown', '1168 TENMILE', '', '14445', 'KNOXVILLE', 'OK', '', '', '', 'active', 'advancepay', '0.00', '', false, false, false, false));
...
/* [4] */ accountList.push(new NewAccount(2085550730, 'Gale', 'Packington', '345 MELROSE ST', '', '44003', 'SUFFOLK', 'FL', '0730', '', 'Iggy Martin', 'lec-inactive', 'advancepay', '0.00', '<tr><td>03/05/2020 10:11:23 AM</td><td>new.trainee Gale Packington / 0730 / Brevard County, FL // pymt $15 - gzv - new bal $8.57</td></tr>', true, true, true, true));
...
/* [5] */ accountList.push(new NewAccount(2085550810, '', '', '', '', '', '', '', '', '', '', 'blocked', 'advancepay', '0.00', '<tr class="red-text"><td>03/05/2020 10:11:23 AM</td><td>bduty changed the account status to BLOCKED.</td></tr>', false, false, false, false));
...
for (i = 0; i < 9; i++){
    ...
    transactionList.push(new NewTransaction(callCost, 'CallUsage',"", 2085550810, 'HOUPASWVALSQL06', date, time));
    transactionList.push(new NewTransaction(callTax, 'Taxes',"", 2085550810, 'HOUPASWVALSQL06', date, time));
}
transactionList.push(new NewTransaction(50.00, 'AdjustmentDecrease',`<span class="yellow-text">GTL -- Adjustment Decrease on Account/Destination 2085550810 and credit card account (411111******1234) is because we received (1) chargeback dispute(s) for a total of ($50.00). The transaction date(s) was/were 10/20/2021. The dispute(s) was/were received ${currDate}. The reason for this dispute/action is "Card Absent Environment."</span>`, 2085550810, 'bduty'));
accountList[5].callList.push({
    date: `${currDate} ${currTime()}`,
    id: facs[1].sub,
    orig: facs[1].orig,
...
/* [6] 2085550720 */ accountList.push(new NewAccount(2085550720, 'Nakita', 'Waybe', '3112 VENTURA DR', '', '32696', 'ENGLEWOOD', 'DC', '0720', 'no email', 'none', 'active', 'advancepay', '0.00', `<tr><td>${currDate} 6:29:21 AM</td><td>new.trainee ms nakita waybe / new acc / pc 0720 / cx ci to xfer funds from wrong number but doesnt remember card - will call back<td><tr>`, true, true, true, false));
transactionList.push(new NewTransaction('0.00', 'NewAccount', "", 2085550720, caresUser, `${currDate}`, `6:26:12 AM`));
...
accountList.push(new NewAccount(2085550710, 'Karen', 'Johnson', '1324 MARIETTA ST', 'APT 21', '95401', 'SANTA ROSA', 'CA', '0710', 'Email: None', 'NONE', 'active', 'advancepay', '0.00', '<tr><td>11/02/2021 9:19:11 AM</td><td>new.trainee karen johnson / cpni passed - pc 0710 / ca-doc  corcoran ca // call dropped after acc su </td></tr>', true, true, false, true));
accountList[8].lastFacility = facs[0].name;
transactionList.push(new NewTransaction('0.00', 'NewAccount',"", 2085550710, caresUser, '11/2/2021', '9:20:58 AM'));
transactionList.push(new NewTransaction('5.00', 'Deposit',"", 2085550710, 'CoMoWeb', '11/2/2021', '10:20:00 AM'));
transactionList.push(new NewTransaction('0.00', 'ServiceFee',"", 2085550710, 'CoMoWeb', '11/2/2021', '10:20:00 AM'));
transactionList.push(new NewTransaction(facs[0].rate, 'CallUsage',"", 2085550710, 'HOUPASWVALSQL06'));
transactionList.push(new NewTransaction(0.01, 'Taxes',"", 2085550710, 'HOUPASWVALSQL06'));
...
accountList.push(new NewAccount('01112085550510', 'Betty', 'Elderly', '111 COPLIN AVE', '', '85012', 'PHOENIX', 'AZ', '', '', '', 'active', 'advancepay', '0.00', '', true, true, true, true));
accountList[9].email = 'belderly@customer.com';
transactionList.push(new NewTransaction('0.00', 'ServiceFee',"", '01112085550510', 'ConnectNetwork', '10/29/2021', '2:23:50 PM'));

/* [10] -  2085550820 rates */ 
accountList.push(new NewAccount(2085550820, 'Jose', 'Davis', '2964 IRVING RD', '', '43150', 'RUSHVILLE', 'OH', '0820', 'no email', 'none', 'active', 'advancepay', '0.00', '<tr><td>11/02/2021 12:21:23 PM</td><td>new.trainee JOSE DAVIS / pc 0820 / OHDRC REFORM // AU: NONE / EMAIL NONE / POL READ / PAY $1 GZV NB $1 </td></tr>', true, true, true, true));
transactionList.push(new NewTransaction('0.00', 'NewAccount',"", 2085550820, caresUser, '11/2/2021', '12:10:11 AM'));
transactionList.push(new NewTransaction('5.00', 'Deposit',"", 2085550820, 'CARES', '11/2/2021', '12:15:11 AM'));
transactionList.push(new NewTransaction('0.00', 'ServiceFee',"", 2085550820, 'CARES', '11/2/2021', '12:15:11 AM'));
...
transactionList.push(new NewTransaction('0.00', 'NewAccount',"", 2085550530, caresUser, currDate, '7:54:11 AM'));
...
transactionList.push(new NewTransaction('0.00', 'NewAccount',"", 2085550620, caresUser, currDate, '7:51:55 AM'));
...
if (document.body.contains(document.getElementById('lookup-button')) == true){
    ...
    document.getElementById('refund-page-no-cc').addEventListener('click', ()=>{
        ...
        if (confirm(msg)){
            ...
            document.getElementById('no-cc-submit').addEventListener('click',()=>{
                transactionList.push(new NewTransaction(refundAmt.value, "AdjustmentDecrease", `Adjust account balance for non-credit card transaction check refund request.`));
    ...
    document.getElementById('container-transactions-search').addEventListener('click', ()=>{
    ...
    document.getElementById('container-records-search').addEventListener('click', ()=>{
        ...
        if (accountList[index].callList != ""){
            ...
            document.getElementById('customer-call-records-body').innerHTML = callRecords;
    ...
    document.getElementById('popup-policy-button1').addEventListener('click', ()=>{
        // Hides the window
        ...
        if (accountList[index].policy3 != CHECK3.checked){
            addNewComment(`${message}PC90DaysAccountExpiration to ${CHECK3.checked}.`);
            accountList[index].policy3 = CHECK3.checked;
        }
    
        if (accountList[index].policy4 != CHECK4.checked){
            addNewComment(`${message}PC180DaysAccountExpiration to ${CHECK4.checked}.`);
            accountList[index].policy4 = CHECK4.checked;
        }    
    });
    
    document.getElementById('popup-policy-button2').addEventListener('click', ()=>{
        // Hides the window
        styleElm('popup-policy-checklist', 'visibility', 'hidden');
        styleElm('popup-policy-checklist', 'left', '-100000px');
    });
    ...
    // ACCOUNT COMMENTS
    document.getElementById('add-new-account-comment').addEventListener('click', ()=>{
            // Shows the window
            styleElm('popup-comment-box', 'visibility', 'visible');
            styleElm('popup-comment-box', 'left', '40%');
    });
    
    document.getElementById('popup-comment-close').addEventListener('click', ()=>{
        // Hides the window
        styleElm('popup-comment-box', 'visibility', 'hidden');
        styleElm('popup-comment-box', 'left', '-100000px');
    });
    
    document.getElementById("popup-comment-submit").addEventListener('click', ()=>{
        // Hides the window
        styleElm('popup-comment-box', 'visibility', 'hidden');
        styleElm('popup-comment-box', 'left', '-100000px');
...
function submitRefund(id){
    ...
    function processRefund(id){
        if (id === 'save'){
            ...
            // add comment
            addNewComment(`added Account Closure request for amount $${refundTotal.value}.`);
            loadAccount(checkAccount());
            ...
            transactionList.push(new NewTransaction(amt.value, 'AdjustmentDecrease', cmt));
...
document.getElementById('adj-increase').addEventListener("click", ()=>{
    ...
    addTransaction.onclick = function(){
        transactionList.push(new NewTransaction(adjAmount.value, "AdjustmentIncrease", adjComment.value));
        styleElm("popup-adj-box", "display", "none");
        loadAccount(checkAccount());
    }
});
...
document.getElementById('adj-decrease').addEventListener("click", ()=>{
    ...
    // apply transaction
    addTransaction.onclick = function(){
        transactionList.push(new NewTransaction(adjAmount.value, "AdjustmentDecrease", adjComment.value));
        if (adjNumber.value != ""){
            transactionList.push(new NewTransaction(adjAmount.value, "AdjustmentIncrease", adjComment.value, adjNumber.value));
        }
        styleElm("popup-adj-box", "display", "none");
        loadAccount(checkAccount());
    }    
...
document.getElementById('adj-deposit').addEventListener('click',()=>{
    ...
    if (document.getElementById('lookup-number').value != ""){
        ...
        changeProperty('popup-cc-deposit-address', 'value', accountList[index].address1);
        ...
        changeProperty('load-text', 'innerHTML', `Please enter a valid account number and click on 'Lookup' before attempting this action.`);    
    }  

});

document.getElementById('popup-cc-deposit-cancel').addEventListener('click', ()=>{
    styleElm('popup-cc-deposit', 'left', '-100000px');
});

document.getElementById('popup-cc-deposit-submit').addEventListener('click', ()=>{
    ...
    } else if (document.getElementById('popup-cc-deposit-address').value == "" || document.getElementById('popup-cc-deposit-city').value == "" || document.getElementById('popup-cc-deposit-zip').value == ""){
        msg.innerHTML = `Full billing address is required.`;
    } else if (document.getElementById('popup-cc-deposit-ccnumber').value == "popup-cc-deposit-ccdate1" || document.getElementById('popup-cc-deposit-cccode').value == "" || document.getElementById('popup-cc-deposit-ccdate1').value == "" || document.getElementById('popup-cc-deposit-ccdate2').value == ""){
...
// SAVE CHANGES
    document.getElementById('form-save-changes').addEventListener('click', ()=>{
        ...
        changeProperty('load-text', 'innerText', `Advance Pay account ${accountList[index].accountNumber} was updated successfully.`);
    })
// SAVE CHANGES
...
function updateCcAuthsTransactions(searchType, searchKey, searchValue=document.getElementById("auths-search").value){
    /*
    ...
    */
    let trans = transactionList.filter(account => account[searchKey] == searchValue && account.ccAuths === true);
    if (document.body.contains(document.getElementById('auths-search2')) == true){
        // if we have a second input, we need to search by both
        let searchValue2 = document.getElementById('auths-search2');
        trans = transactionList.filter(account => account[searchKey] == searchValue && account.last4 == searchValue2.value && account.ccAuths === true);
    }
    ...
    if (trans.length != 0){
        ...
        document.getElementById('cc-auths-error').innerText = "No records found. Please change search criteria and try again."
        let emptyRows = ``;
        if (searchType != "destination"){
            head.innerHTML = elseHead;
            emptyRows += `<td></td>`;
...
document.querySelector('#nav-cca').addEventListener('click', ()=>{
    ...
    if (accountNumber.value != ""){
        ...
        body.innerHTML = updateCcAuthsTransactions('destination', "accountNumber", accountNumber.value);;
    } else {
...
document.querySelector('#auths-button1').addEventListener('click',()=>{
    ...
    if (searchType.value == "destination"){
        document.querySelector('#cc-auths-body').innerHTML = updateCcAuthsTransactions('destination', "accountNumber");
    } else {
        document.querySelector('#cc-auths-body').innerHTML = updateCcAuthsTransactions(searchType.value, searchType.value);
    }
